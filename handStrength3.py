...
test = [(7, 'd'), (5, 's'), (9, 's'), (5, 'd'), (5, 'd'), (12, 'd'), (8, 's')]
...
#    print(how_many)
	for suit in ['c', 'h', 'd', 's']:
		...
		if how_many[suit] > 4:
			rank = 1
			if hand[0][0] > rank and hand[0][1] == suit:
				rank = hand[0][0]
			if hand[1][0] > rank and hand[1][1] == suit:
				rank = hand[1][0]
...
def pair(hand):
 ...
 #   print('Pair count123: ', pair_count)
...
def returning(x = test):
	global achieved_strength, achieved_strength2, high_hand
#    print('returning is being called with: ', x)
	achieved_strength = {'straight_flush':0, 'four_kind':0, 'full_house':0,
					 'flush':0, 'straight':0, 'three_kind':0,
					 'two_pair':0, 'pair':0,'high_card':0}
	achieved_strength2 = []
...
##    print ('High hand: ', high_hand)
	return achieved_strength, achieved_strength2, high_hand

def ach_s():
	return achieved_strength

'''
if __name__ == '__main__': flush(test), straight(test), four_kind(test),
three_kind(test), pair(test), high_card(test)
'''
if __name__ == '__main__': print(returning(x = test))
